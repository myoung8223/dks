...
// Runs a command in its OWN visible console window (no hidden process).
// Used for the upload step, where live arduino-cli progress is useful and
// there is no output to capture.
int RunCommandVisible(const char* cmd) {
    STARTUPINFO si;
    ...
    if (CreateProcess(NULL, cmdBuffer, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
...
// Runs a command in a VISIBLE console, tees its combined stdout/stderr live to
// that console for transparency, and mirrors the output to a temp file so the
// tail can be captured for parsing (the flash-size guard needs the
// "Sketch uses N bytes" line). No hidden process, no output-suppression flags.
int RunCommandCapture(const char* cmd, char* outBuf, int outBufSize) {
    ...
    FILE* mirror = fopen(tmpFile, "wb");
    if (!mirror) { DeleteFile(tmpFile); return -1; }

    // Pipe: child writes stdout+stderr into the write end; the parent reads the
    // read end and tees each chunk to the console and to the temp file.
    ...
    if (!CreatePipe(&hRead, &hWrite, &sa, 0)) {
        fclose(mirror); DeleteFile(tmpFile); return -1;
    }
    SetHandleInformation(hRead, HANDLE_FLAG_INHERIT, 0); // keep read end private

    // Give this (GUI) process a real console window so the build is visible.
    // If a console already exists (e.g. run from a dev cmd), AllocConsole fails
    // harmlessly and we reuse the existing one.
    ...
    int exitCode = -1;
    if (CreateProcess(NULL, cmdBuffer, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
        CloseHandle(hWrite); hWrite = NULL; // parent must close its copy to see EOF

        char buf[512];
        DWORD n = 0;
        while (ReadFile(hRead, buf, sizeof(buf), &n, NULL) && n > 0) {
            fwrite(buf, 1, n, mirror);
            if (hConsole && hConsole != INVALID_HANDLE_VALUE) {
                DWORD written;
                WriteFile(hConsole, buf, n, &written, NULL);
            }
        }
    ...
    // Read the TAIL of the captured output (arduino-cli prints the "Sketch uses"
    // / "Global variables" size lines near the end, so keeping the tail preserves
    // them even after a build with many warnings).
    if (outBuf && outBufSize > 0) {
        ...
        if (fp) {
            ...
            if (fsize > want) fseek(fp, fsize - want, SEEK_SET);
            else rewind(fp);
            size_t r = fread(outBuf, 1, (size_t)outBufSize - 1, fp);
            outBuf[r] = '\0';
            fclose(fp);
        }
    }
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDM_FLASH: {
                    ...
                    if (transpile("temp_payload.txt")) {
                        ...
                        int uploadStatus = RunCommandVisible("arduino-cli upload --config-file arduino-cli.yaml --fqbn digistump:avr:digispark-tiny sketch");
